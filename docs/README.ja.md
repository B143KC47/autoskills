...
- **マルチエージェント深層探索** —— Claude Code ではオンライン探索が `Workflow` として実行されます。複数の Sonnet ファインダーが 4 つの角度（エコシステム、リーダーボード、GitHub、新着リリース）を並列に走査し、Opus 検証エージェントが各候補の実際のコンテンツを取得したうえで敵対的にスコア付けします。Codex では **Codex サブエージェント**（軽量モデルの並列ファインダー + 強力モデルの検証）にマッピングされ、サブエージェントを全く持たない環境では完全に逐次的なフローになります。
...
| 🟡 **Agent** | サブエージェント対応の環境 —— **Codex サブエージェント**（2026 年 GA）を含む | 並列ファインダーワーカー（各軽量モデル、環境のスレッド上限まで）+ 単一の最強モデル検証エージェント |
| 🔵 **Inline** | サブエージェントなし（制限付きサンドボックス、旧版 CLI） | 同じ角度とルーブリックを、エージェント自身が逐次実行 |
