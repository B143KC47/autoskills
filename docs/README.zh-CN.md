...
- **多智能体深度搜索** —— 在 Claude Code 中，在线发现以 `Workflow` 方式运行：多个 Sonnet 查找器并行扫描四个角度（生态、排行榜、GitHub、新发布），随后由 Opus 验证器逐一对抗式评分（先抓取候选技能的真实内容再判断）。在 Codex 中映射为 **Codex 子智能体**（轻量模型并行查找 + 强模型验证）；完全没有子智能体的环境则退化为顺序化流程。
...
| 🟡 **Agent** | 任何支持子智能体的环境 —— 含 **Codex 子智能体**（2026 正式发布） | 并行查找器（每个用轻量模型，受环境线程上限约束）+ 单个最强模型验证智能体 |
| 🔵 **Inline** | 无子智能体（受限沙箱、旧版 CLI） | 相同的角度与评分标准，由智能体自身顺序执行 |
