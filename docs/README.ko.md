...
- **멀티 에이전트 심층 검색** —— Claude Code에서는 온라인 탐색이 `Workflow`로 실행됩니다. 여러 Sonnet 파인더가 네 가지 각도(생태계, 리더보드, GitHub, 신규 릴리스)를 병렬로 훑고, Opus 검증 에이전트가 각 후보의 실제 콘텐츠를 가져와 적대적으로 점수화합니다. Codex에서는 **Codex 서브에이전트**(경량 모델 병렬 파인더 + 강력 모델 검증)로 매핑되며, 서브에이전트가 전혀 없는 환경에서는 완전히 순차적인 흐름으로 동작합니다.
...
| 🟡 **Agent** | 서브에이전트를 지원하는 모든 환경 —— **Codex 서브에이전트**(2026 GA) 포함 | 병렬 파인더 워커(각각 경량 모델, 환경의 스레드 상한까지) + 단일 최강 모델 검증 에이전트 |
| 🔵 **Inline** | 서브에이전트 없음(제한된 샌드박스, 구버전 CLI) | 같은 각도와 루브릭을 에이전트가 직접 순차 실행 |
