...
- **多代理深度搜尋** —— 在 Claude Code 中，線上探索以 `Workflow` 方式執行：多個 Sonnet 尋找器並行掃描四個角度（生態、排行榜、GitHub、新發布），再由 Opus 驗證器逐一對抗式評分（先擷取候選技能的真實內容再判斷）。在 Codex 中對應為 **Codex 子代理**（輕量模型並行尋找 + 強模型驗證）；完全沒有子代理的環境則退化為循序流程。
...
| 🟡 **Agent** | 任何支援子代理的環境 —— 含 **Codex 子代理**（2026 正式推出） | 並行尋找器（每個用輕量模型，受環境執行緒上限約束）+ 單一最強模型驗證代理 |
| 🔵 **Inline** | 無子代理（受限沙箱、舊版 CLI） | 相同的角度與評分標準，由代理自身循序執行 |
