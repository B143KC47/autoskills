...
- Content unreadable: empty folder, OR a broken/`unsynced` symlink (e.g., an entry linking into a skill store not synced to this machine). Never recommend a skill whose content can't be read; you may surface its name at most as "catalog: currently unavailable".
...
## Tiers
**Fit gate:** if Fit = 0 (wrong domain / wrong task), the candidate is **No fit — drop it**, whatever its total. Trust, freshness, and popularity cannot compensate for the wrong domain.

For candidates with Fit ≥ 1:
